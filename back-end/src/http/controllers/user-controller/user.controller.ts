import {Body, Controller, Post, Delete, Put, Param, UseGuards, Request} from "@nestjs/common";
import {CreateUserUseCase} from "../../../modules/user/use-cases/create-user-use-case/create-use-user-case";
import {ChangeUserRoleDto, CreateUserBody} from "../../../modules/user/dtos/create-user-body";
...
export class UserController {
    ...
    async deleteUser(@Param('id') id: string, @Request() request: AuthenticatedRequestModel) {
    ...
    @Put(':id')
    @UseGuards(JwtAuthGuard, new RolesGuard('admin'))
    async updateUser(@Param('id') id: string, @Body() userData: Partial<User>, @Request() request: AuthenticatedRequestModel) {
